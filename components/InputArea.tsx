...
const InputArea: React.FC<InputAreaProps> = ({
    input, setInput, handleSend, stopGeneration, files, onRemoveFile, onFileSelect,
    ...
    useEffect(() => {
        ...
        if (el) {
            ...
            el.style.height = input ? Math.min(el.scrollHeight, 120) + 'px' : '40px';
        }
    }, [input]);
                ...
                <div className={`relative flex flex-col p-2 rounded-3xl shadow-sm border transition-all focus-within:ring-2 focus-within:ring-zinc-300 dark:focus-within:ring-zinc-600 bg-white/95 dark:bg-zinc-900/95 border-zinc-200 dark:border-zinc-800`}>
                            ...
                            {files.map((f, i) => (
                                <div key={i} className="relative shrink-0 w-16 h-16 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 overflow-hidden group">
                                    {f.type.startsWith('image/') 
                                        ? <img src={f.content} className="w-full h-full object-cover opacity-90" alt="Preview" />
                    ...
                    <div className="flex items-end gap-2">
                        ...
                        <div className="relative shrink-0 w-10 h-10 flex items-center justify-center">
                            <input type="file" multiple onChange={onFileSelect} disabled={isLoading} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" accept={mode === 'img2txt' ? "image/*" : (mode === 'speech2txt' ? "audio/*" : "*/*")} title="Attach files" />
                            <button disabled={isLoading} className="w-10 h-10 rounded-full flex items-center justify-center transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500 dark:text-zinc-400 active:bg-zinc-200 dark:active:bg-zinc-700"><Icons.Paperclip size={20} /></button>
                        </div>
                        ...
                        <div className="flex items-center gap-1 pb-0">
                            {/* Mic Button */}
                            {mode !== 'img2txt' && mode !== 'speech2txt' && files.length === 0 && isSpeechSupported && !isLoading && (
                            ...
                            {/* Send/Stop Button */}
                            {isLoading ? (
                                <button onClick={stopGeneration} title="Stop generation" className="w-10 h-10 rounded-full flex items-center justify-center transition-all bg-zinc-800 dark:bg-zinc-200 text-white dark:text-black hover:scale-105 active:scale-95 shadow-md"><Icons.Square size={14} className="fill-current" /></button>
                            ) : (
                                <button onClick={handleSend} title="Send message" disabled={(!input.trim() && files.length === 0)} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all active:scale-95 ${ (input.trim() || files.length > 0) ? 'bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 shadow-md hover:shadow-lg' : 'bg-zinc-200 dark:bg-zinc-800 text-zinc-400 opacity-50 cursor-not-allowed'}`}><Icons.Send size={18} /></button>
                            )}
                        </div>
                    </div>
                </div>
